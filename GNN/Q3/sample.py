...
from utils import sparse_mx_to_torch_sparse_tensor, data_loader
...
class Sampler:
    ...
    def __init__(self, dataset, device):
        ...
        (self.adj,
         ...
         self.idx_test, 
         self.degree,
         self.learning_type) = data_loader(dataset, "NoNorm")
    ...
    def _preprocess_adj(self, normalization, adj):
    ...
    def stub_sampler(self, normalization):
        """
        ...
        if normalization in self.trainadj_cache:
            ...
            r_adj = self._preprocess_adj(normalization, self.train_adj)
            self.trainadj_cache[normalization] = r_adj
        fea = self._preprocess_fea(self.train_features)
        return r_adj, fea

    def randomedge_sampler(self, percent, normalization):
        """
        ...
        r_adj = self._preprocess_adj(normalization, r_adj)
        fea = self._preprocess_fea(self.train_features)
        return r_adj, fea
    ...
    def vertex_sampler(self, percent, normalization):
        ...
        r_adj = self._preprocess_adj(normalization, r_adj)
        r_fea = self._preprocess_fea(r_fea)
    ...
    def get_test_set(self, normalization):
        """
        Return the test set. 
        """
        if self.learning_type == "transductive":
            ...
            if normalization in self.adj_cache:
                ...
                r_adj = self._preprocess_adj(normalization, self.adj)
                self.adj_cache[normalization] = r_adj
            fea = self._preprocess_fea(self.features)
            return r_adj, fea

    def get_val_set(self, normalization):
        """
        Return the validataion set. Only for the inductive task.
        Currently behave the same with get_test_set
        """
        return self.get_test_set(normalization)

    def get_label_and_idxes(self):
        """
        Return all labels and indexes.
        """
        return self.labels_torch.to(self.device), self.idx_train_torch.to(self.device), \
                self.idx_val_torch.to(self.device), self.idx_test_torch.to(self.device)
